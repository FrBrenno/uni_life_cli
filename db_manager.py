...
def create_course_tracking_tables(conn, cursor):
    ...
    create_course_labo_table(conn, cursor)
...
def create_course_labo_table(conn, cursor):
    # Check if the table 'course_labos' exists
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='course_labos'")
    table_exists = cursor.fetchone()
    if not table_exists:
        # Table doesn't exist, so create it
        cursor.execute('''CREATE TABLE IF NOT EXISTS course_labos
                          (id INTEGER PRIMARY KEY AUTOINCREMENT,
                           course_id INTEGER,
                           labo_number INTEGER,
                           labo_name TEXT,
